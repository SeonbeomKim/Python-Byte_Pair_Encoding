...
def _learn_bpe(word_frequency_dict, voca_out_path, num_merges=37000, multi_proc=1):
	...
	# make npy
	dirname = os.path.dirname(voca_out_path)
	#filename = os.path.basename(voca_out_path)
	if not os.path.exists(dirname):
		print("create" + dirname + "directory")
		os.makedirs(dirname)

	voca_from_learn_BPE = get_vocabulary_from_learn_BPE(word_frequency)
	save_voca(voca_out_path, voca_from_learn_BPE)

	print('save', voca_out_path, ', size:', len(voca_from_learn_BPE))

	

def learn_bpe(path_list, voca_out_path, space_symbol='</w>', num_merges=37000, voca_threshold=5, multi_proc=1):
